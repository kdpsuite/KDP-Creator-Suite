...
class RateLimiter:
    ...
    def _cleanup(self):
        ...
        for key in list(self.requests.keys()):
            self.requests[key] = [timestamp for timestamp in self.requests[key] if timestamp > cutoff_time]
...
def rate_limit(max_requests, window_seconds, key_func=None):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            # Check rate limit
            allowed, remaining, reset_time = _rate_limiter.is_allowed(key, max_requests, window_seconds)
...
def rate_limit_password_reset(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        allowed, remaining, reset_time = _rate_limiter.is_allowed(
            f"password_reset:{email}", PASSWORD_RESET_LIMIT[0], PASSWORD_RESET_LIMIT[1]
        )
